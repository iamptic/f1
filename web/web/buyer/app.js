(() => {
  ...
  const geo = {
    get: () => { try{ return JSON.parse(localStorage.getItem("buyer_geo")||"null"); }catch{ return null; } },
  ...
  const loadFilters = () => {
    try{
      const j = localStorage.getItem(FILTERS_KEY); if(!j) return;
      const s = JSON.parse(j); Object.assign(state, s||{});
    }catch{}
  };

  // Data
  let offers = []; let total=null, offset=0, LO=12, loading=false;
  let currentOffer = null; let currentQty = 1;
  ...
  const labelCat = (c) => { if(!c) return 'Другое'; const key=String(c).toLowerCase(); return CATEGORY_LABELS[key] || (key[0].toUpperCase()+key.slice(1)); };
    ...
    list.forEach(o => {
      ...
      el.innerHTML = `
        ...
        <div class="p">
          <div class="price">
            ${money(o.price_cents)}
            ${o.original_price_cents?`<span class="old">${money(o.original_price_cents)}</span>`:''}
  ...
  // Extract code
  function _pickCode(obj){ if (!obj) return ''; if (typeof obj==='string') return obj; return obj.code||obj.qr_code||obj.reservation_code||obj.token||''; }
  ...
  function getQrPayload(data){ try{ return (data.qr_url || data.url || '').trim(); }catch(_){ return ''; } }

  // Offer sheet + reserve + QR
  function openOffer(o){
    currentOffer = o; currentQty = 1; if (qtyInput) qtyInput.value = '1';
    $('#sTitle').textContent = o.title||'—';
    $('#sImg').src = o.image_url||'';
    $('#sPrice').textContent = money(o.price_cents);
    const old=(o.original_price_cents||0)/100; $('#sOld').textContent=old? (Math.round(old)+' ₽'):'—';
  ...
  function clampQty(n){
    const max = (currentOffer?.qty_left ?? 999) || 999;
    return Math.max(1, Math.min(max, n|0));
  }
  if (qtyPlus) qtyPlus.onclick = () => { currentQty = clampQty((currentQty||1)+1); qtyInput.value = String(currentQty); };
  if (qtyMinus) qtyMinus.onclick = () => { currentQty = clampQty((currentQty||1)-1); qtyInput.value = String(currentQty); };
  if (qtyInput) qtyInput.oninput = () => { currentQty = clampQty(parseInt(qtyInput.value||'1',10)||1); qtyInput.value = String(currentQty); };
  ...
  $('#reserveBtn').onclick = async () => {
    ...
    try{
      const resp = await fetch(API + '/api/v1/public/reserve', {
        ...
        body: JSON.stringify({ offer_id: o.id || o.offer_id, name:'Гость', phone:'', quantity: currentQty })
      });
      ...
      if (it && typeof it.qty_left==='number') it.qty_left = Math.max(0, it.qty_left-currentQty);
      render();
    ...
    setTimeout(()=>{
      try{
        if (window.QRCode && typeof window.QRCode.toCanvas==='function'){
          window.QRCode.toCanvas(canvas, code, { width:220, margin:1, errorCorrectionLevel:'M' }, ()=>{});
